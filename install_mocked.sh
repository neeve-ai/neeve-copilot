...
if [ ! -d "$INSTALL_DIR" ]; then
    ...
    echo "Mock git clone" --depth 1 "$REPO_URL" "$INSTALL_DIR"
else
    printf "%b\n" "${YELLOW}🔄 Updating Neeve repo...${NC}"
    cd "$INSTALL_DIR" && echo "Mock git fetch" --quiet --depth=1 origin main && echo "Mock git reset" --quiet --hard FETCH_HEAD && cd - > /dev/null
fi

# 2. Clone/Update Awesome-Copilot (Dependency)
if [ ! -d "$AWESOME_DIR" ]; then
    printf "%b\n" "${YELLOW}📥 Fetching awesome-copilot base...${NC}"
    echo "Mock git clone" --depth 1 "$AWESOME_URL" "$AWESOME_DIR"
else
    printf "%b\n" "${YELLOW}🔄 Updating awesome-copilot base...${NC}"
    cd "$AWESOME_DIR" && echo "Mock git fetch" --quiet --depth=1 origin main && echo "Mock git reset" --quiet --hard FETCH_HEAD && cd - > /dev/null
fi
