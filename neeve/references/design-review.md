# Design Review — DLS Fidelity, Accessibility, Failure States

Referenced by `neeve-dls` and by `code-review` when a change touches a
customer-facing UI surface. `neeve-dls` implements DLS-faithful changes; this
reference is the review lens applied to that work (or to a design proposal
before implementation) — the same pixel-perfect bar, plus two things a pure
implementation pass won't independently check: accessibility as a compliance
surface, and failure-state design for operational UI. Full reasoning:
`engineering-principles.md` § Design Principles.
...
5. **Ground WCAG/VPAT criteria for real when a finding hinges on the exact
   ...
   confirm the exact criterion rather than citing a remembered version of
   it — an accessibility finding that misquotes the actual standard
   undermines the whole review with an enterprise buyer's compliance team.
